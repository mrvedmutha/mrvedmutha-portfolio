...
export default function ExperienceSection() {
  ...
  useEffect(() => {
    getExperiences().then((data) => {
      // Sort by fromDate descending (latest first)
      setExperiences(
        data.sort(
          (a, b) =>
  ...
  const showExperiences = experiences.slice(0, 4);
  const hasMore = experiences.length > 4;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {showExperiences.map((exp, idx) => (
          <motion.div
            ...
            className="bg-muted rounded-xl shadow p-8 flex flex-col justify-center items-start border border-border min-h-[220px] relative"
          >
            {/* Job Title & Company */}
            <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
              <span className="text-xl font-bold">{exp.jobTitle}</span>
              <span className="text-muted-foreground">@ {exp.companyName}</span>
            </div>
            {/* Dates */}
